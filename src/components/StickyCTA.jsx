...
export default function StickyCTA() {
  ...
  return (
    <>
      ...
      <div
        className={`fixed bottom-6 right-4 z-[55] flex flex-col gap-2.5 transition-all duration-300 sm:hidden ${
          visible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0 pointer-events-none'
        }`}
      ...
      >
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg shadow-green-500/25 transition-all duration-200 hover:scale-110 active:scale-95"
          aria-label="WhatsApp"
        >
          <FaWhatsapp className="text-xl" />
        </a>
        <Link
          to="/contact"
          className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-[#1d1d1f] text-white shadow-lg shadow-black/20 transition-all duration-200 hover:scale-110 active:scale-95"
          aria-label="Book Demo"
        >
          <HiOutlineCalendarDays className="h-5 w-5" strokeWidth={1.5} />
        </Link>
        <Link
          to="/signup"
          className="inline-flex h-11 items-center gap-2 rounded-full bg-[#0071e3] px-5 text-sm font-medium text-white shadow-lg shadow-blue-500/25 transition-all duration-200 hover:scale-105 active:scale-95"
        >
          <HiOutlinePlayCircle className="h-5 w-5" />
          Start Free
          <HiOutlineArrowRight className="h-4 w-4" />
