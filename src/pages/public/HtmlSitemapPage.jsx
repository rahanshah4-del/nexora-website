      ...
      <section className="relative overflow-hidden bg-[linear-gradient(180deg,#ffffff_0%,#f8fbff_60%,#f1f5f9_100%)] py-16 sm:py-20 lg:py-24">
        ...
        <div className="relative mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="mt-6 text-4xl font-bold tracking-[-0.02em] text-slate-950 sm:text-5xl">Find every public Nexora page.</h1>
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-3">
            {sitemapGroups.map((group) => (
              <section key={group.title} className="rounded-[1.2rem] border border-slate-200/60 bg-white p-6 shadow-[0_4px_20px_-8px_rgba(15,23,42,0.05)]">
                <h2 className="text-[17px] font-semibold tracking-[-0.01em] text-slate-900">{group.title}</h2>
                <div className="mt-4 grid gap-2.5">
                  {group.links.map(([label, to]) =>
                    to.startsWith('/sitemap') || to.startsWith('/blog-sitemap') || to.startsWith('/rss') ? (
                      <a key={label} href={to} className="text-[13px] font-medium text-slate-600 transition-colors hover:text-slate-900">
                        {label}
                      </a>
                    ) : (
                      <Link key={label} to={to} className="text-[13px] font-medium text-slate-600 transition-colors hover:text-slate-900">
                        {label}
                      </Link>
                    )
                  )}
          ...
          <section className="mt-8 rounded-[1.2rem] border border-slate-200/60 bg-white p-6 shadow-[0_4px_20px_-8px_rgba(15,23,42,0.05)]">
            <h2 className="text-[17px] font-semibold tracking-[-0.01em] text-slate-900">Latest Blog Articles</h2>
            <div className="mt-4 grid gap-2.5 md:grid-cols-2">
              {blogArticles.map((article) => (
                <Link key={article.slug} to={`/blog/${article.slug}`} className="text-[13px] font-medium leading-6 text-slate-600 transition-colors hover:text-slate-900">
                  {article.title}
                </Link>
              ))}
            </div>
