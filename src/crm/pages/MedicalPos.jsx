...
function todayShiftKey(workspaceId) {
  ...
  return `nexora.medicalPos.shift.${workspaceId || 'local'}.${day}`
}

function loadMedicalPosShift(workspaceId) {
...
function saveMedicalPosShift(workspaceId, shift) {
  if (typeof window === 'undefined') return null
  const clean = {
...
function buildMedicalPosThermalText(order = {}) {
...
export default function MedicalPosPage() {
  const { workspaceId, businessType, userDoc, firebaseUser, userId, isOwner, isAdmin, isStaff } = useUser()
  const { settings: businessSettings } = useBusinessSettings()
  const medicineApi = useMedicineInventory({ limitCount: 200 })
  const ordersApi = useMedicalPosOrders({ limitCount: 8 })
  ...
  useEffect(() => {
    setShift(loadMedicalPosShift(workspaceId))
  }, [workspaceId])

  useEffect(() => {
    window.history.pushState({ nexoraMedicalPosTill: true }, '', window.location.href)
    function handleBackButton() {
      const leave = window.confirm('Medical POS Billing is open. Going back can close this till screen. Open another page in a separate window/tab instead. Press OK only if you want to leave Medical POS Billing.')
      if (!leave) {
        window.history.pushState({ nexoraMedicalPosTill: true }, '', window.location.href)
        return
      }
  ...
  const medicines = useMemo(
    () => medicineApi.medicines.filter((medicine) => medicine.status !== 'inactive' && medicine.productType !== 'service'),
    [medicineApi.medicines],
  )
  const categories = useMemo(() => ['All', ...Array.from(new Set(medicines.map((medicine) => medicine.category || 'General')))], [medicines])
  const filteredMedicines = useMemo(() => {
    const needle = query.trim().toLowerCase()
    return medicines
      .filter((medicine) => category === 'All' || medicine.category === category)
      .filter((medicine) => !needle
  ...
  useEffect(() => {
    const defaultMethod = medicalPosSettings.defaultPaymentMethod || ''
    if (defaultMethod && paymentMethod === 'Cash') setPaymentMethod(defaultMethod)
  }, [paymentMethod, medicalPosSettings.defaultPaymentMethod])
  ...
  const addMedicine = useCallback((medicine) => {
    ...
    const existing = cart.find((item) => item.productId === medicine.id)
    const nextQty = (existing?.quantity || 0) + 1
    if (stock <= 0 || nextQty > stock) {
      setMessage(`${medicine.name} stock available nahi hai.`)
      return
    }
    setCart((current) => {
      if (existing) {
  ...
  const addScannedCode = useCallback((rawCode) => {
    ...
    if (!medicine) {
      setQuery(code)
      setMessage(`No medicine found for barcode/SKU: ${code}`)
      return false
    }
    addMedicine(medicine)
    setQuery('')
    setMessage(`${medicine.name} scanned and added to cart.`)
    return true
  }, [addMedicine, businessSettings?.barcodeScannerSettings, medicines])
  ...
  async function addCustomerFromTill() {
    ...
    const result = await customersApi.createCustomer({
      ...
      status: 'Active',
      notes: 'Created from Medical POS Billing.',
    })
    if (result?.ok) {
  ...
  async function submitOrder(shouldPrint = false, shiftOverride = null) {
    ...
    if (!cart.length) {
      setMessage('Cart empty hai. Pehle medicine add karein.')
      return
    }
    if (!workspaceId || !db) {
    ...
    const activeShift = shiftOverride || shift || loadMedicalPosShift(workspaceId)
    if (!activeShift) {
      setPendingPrint(Boolean(shouldPrint))
      setShiftDraft({ openingCash: String(medicalPosSettings.defaultOpeningCash || ''), note: medicalPosSettings.cashDrawerName || '' })
    ...
    try {
      ...
      const result = await ordersApi.createOrder({
        ...
        branch: 'Main Branch',
        branchId: medicalPosSettings.branchId || '',
        registerId: activeShift.id,
        cashier: cashierName,
        cashierId,
        staffId: cashierId,
        cashierName,
        moduleKey: 'medical_pos',
        orderSource: 'medical_pos_front_till',
      ...
      if (!result?.ok) {
        throw new Error(result?.error || 'Medical POS order save nahi ho saka.')
      }
      ...
      const queueCollection = collection(db, workspaceCollectionPath(workspaceId, 'medicalPosQueueJobs'))
      ...
      cartSnapshot.forEach((item) => {
        ...
        const medicineRef = doc(db, workspaceCollectionPath(workspaceId, 'medicineInventory'), item.productId)
        batch.update(medicineRef, {
          stockQuantity: newStock,
          stockHistory: [
            ...(medicines.find((medicine) => medicine.id === item.productId)?.stockHistory || []),
            {
              type: 'sale',
              quantity: item.quantity,
              ...
              delta: -item.quantity,
              note: `Medical POS sale ${orderNumber}`,
      ...
      Promise.resolve().then(async () => {
        ...
        try {
          ...
          console.warn('[Medical Store POS] order saved but stock sync failed', {
            orderNumber,
            code: syncError?.code || '',
      ...
      setMessage(error?.message || 'Medical POS order save nahi ho saka.')
    } finally {
      if (unlockTimer) window.clearTimeout(unlockTimer)
      savingRef.current = false
      setSaving(false)
  ...
  function startShiftAndContinue(event) {
    ...
    const nextShift = saveMedicalPosShift(workspaceId, {
      openingCash,
  ...
  async function printReceipt(order) {
      ...
      return `
        <tr>
          <td>
            ...
            <small>${escapeHtml(item.sku || item.barcode || '')}${item.batchNumber ? ` · Batch ${escapeHtml(item.batchNumber)}` : ''}</small>
          </td>
          <td>${qty}</td>
    ...
    const thermalText = buildMedicalPosThermalText({ ...order, companyName, phone, address, footer, date })
        ...
        <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/35 px-4 py-6">
          <form onSubmit={startShiftAndContinue} className="w-full max-w-md rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-2xl">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-600">Start Medical POS Till</p>
          ...
          <Card className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-[1.4rem] border-slate-200/80 bg-white p-4">
            <div className="shrink-0 rounded-2xl border border-slate-100 bg-gradient-to-r from-slate-50 to-blue-50/60 px-3 py-2.5">
              ...
              <div>
                ...
                <p className="text-xs font-semibold text-slate-500">{cart.length} medicines · {cart.reduce((sum, item) => sum + item.quantity, 0)} items</p>
              </div>
