...
export default function MedicalPosOrdersPage() {
  ...
  const canDeleteOrders = access.isOwner || access.isAdmin || access.hasModulePermission('medicalPosOrders', 'delete') || access.hasModulePermission('medicalPos', 'delete')
  ...
  function printOrder(order) {
    const rows = (order.items || []).map((item) => `
      <tr><td><span class="item-name">${escapeHtml(item.name)}</span><br><span class="item-qty">x${Number(item.quantity || 0)} @ ${formatCurrency(Number(item.price || 0))}${item.batchNumber ? ` · Batch ${escapeHtml(item.batchNumber)}` : ''}</span></td><td class="num">${formatCurrency(Number(item.lineTotal || item.price * item.quantity || 0))}</td></tr>
    `).join('')
    ...
    const html = `<!doctype html><html><head><title>${escapeHtml(order.orderNumber)}</title><style>
      ...
      <div class="center">
        ...
        <div class="sub-brand">MEDICAL STORE INVOICE</div>
  ...
  async function handleDeleteOrder(order) {
    const ok = await confirmAction({
      ...
      title: 'Delete medical POS order?',
      message: `This will remove ${order.orderNumber} from Medical POS Orders and sales reports. Stock will not be restored automatically. Use Medicine Inventory stock movement if stock needs correction.`,
  ...
  function printTodayReport() {
    ...
    `).join('')
    const html = `<!doctype html><html><head><title>Medical POS Today Report</title><style>
      ...
      <div class="divider"></div>
      ${todayOrders.length ? rows : '<div class="empty">No Medical POS orders found today.</div>'}
  ...
  return (
    <motion.div className="min-w-0 space-y-5" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22 }}>
      <PageHeader
        title="Medical POS Orders"
        subtitle="Medical store front-till orders are stored separately from invoices and do not enter approval centre."
        right={
          <div className="flex flex-wrap gap-2">
            ...
            <Button type="button" className="rounded-2xl" onClick={() => window.open('/app/medical-pos', '_blank', 'noopener,noreferrer')}>
              <HiOutlineBolt className="h-4 w-4" /> Open Medical POS Billing
            </Button>
          </div>
        }
      />
      ...
      <Card className="rounded-[1.4rem] border-slate-200/80 bg-white p-4">
        ...
        {loading ? (
          <div className="rounded-2xl border border-dashed border-slate-200 py-12 text-center text-sm font-semibold text-slate-500">Loading Medical POS orders...</div>
        ) : orders.length ? (
          <>
          ...
          <div className="rounded-2xl border border-dashed border-slate-200 py-12 text-center text-sm font-semibold text-slate-500">No Medical POS orders yet.</div>
        )}
      </Card>
    </motion.div>
  )
