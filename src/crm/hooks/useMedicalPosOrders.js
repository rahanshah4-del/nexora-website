...
function mergeOrders(remoteRows, localRows, limitCount) {
  ...
  ;[...localRows, ...remoteRows].forEach((row) => {
    const normalized = normalizeMedicalPosOrder(row)
...
export function normalizeMedicalPosOrder(order = {}) {
  ...
  return {
    ...
    notes: order.notes || '',
    source: order.source || 'medical_pos',
    orderSource: order.orderSource || order.source || 'medical_pos_front_till',
    moduleKey: order.moduleKey || 'medical_pos',
...
export function useMedicalPosOrders(options = {}) {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeUserCollection(
      ...
      (err) => {
        setError(clientSafeMessage(err, 'Unable to load medical POS orders.'))
        setOrders(mergeOrders([], readLocalOrders(workspaceId), limitCount))
        setLoading(false)
      },
      {
  ...
  useEffect(() => {
    ...
    window.addEventListener('nexora:medical-pos-orders-updated', refreshLocalOrders)
    window.addEventListener('storage', refreshStorageOrders)
    return () => {
      window.removeEventListener('nexora:medical-pos-orders-updated', refreshLocalOrders)
  ...
  const createOrder = useCallback(async (payload) => {
    ...
    const canCreate = access.isOwner || access.isAdmin || access.hasModulePermission('medicalPosOrders', 'create') || access.hasModulePermission('medicalPos', 'create')
    ...
    if (submittingRef.current) {
      console.warn('[Medical Store POS] duplicate createOrder call blocked (already submitting)')
    ...
    try {
      ...
      createUserDoc(workspaceId, 'medicalPosOrders', firestorePayload, {
        businessType: effectiveBusinessType,
      }).then((ref) => {
  ...
  const deleteOrder = useCallback(async (id) => {
    ...
    if (!workspaceId || !userId) return { ok: false, error: 'Please login first.' }
    if (!db) return { ok: false, error: 'Secure Cloud Sync is not available right now.' }
    const canDelete = access.isOwner || access.isAdmin || access.hasModulePermission('medicalPosOrders', 'delete') || access.hasModulePermission('medicalPos', 'delete')
    ...
    try {
      ...
      await removeUserDoc(workspaceId, 'medicalPosOrders', id)
  ...
  const refundOrder = useCallback(async (id) => {
    ...
    if (!workspaceId || !userId) return { ok: false, error: 'Please login first.' }
    if (!db) return { ok: false, error: 'Secure Cloud Sync is not available right now.' }
    const canRefund = access.isOwner || access.isAdmin || access.hasModulePermission('medicalPosOrders', 'create') || access.hasModulePermission('medicalPos', 'create')
    ...
    try {
      ...
      const orderRef = doc(db, workspaceCollectionPath(workspaceId, 'medicalPosOrders'), id)
      const refundTxnRef = doc(db, workspaceCollectionPath(workspaceId, 'accountTransactions'), `refund-medical-pos-${id}`)
      ...
      if (refundItems.length) {
        ...
        if (!restoreRs.ok) {
          ...
          console.warn('[Medical POS Refund] inventory restore had errors', restoreRs.errors)
        }
      }
