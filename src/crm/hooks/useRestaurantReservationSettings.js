...
export function useRestaurantReservationSettings({ enabled = true } = {}) {
  const { workspaceId, businessType, userId } = useUser()
  const access = useWorkspaceAccess()
  const [settings, setSettings] = useState(RESERVATION_SETTINGS_DEFAULTS)
  ...
  useEffect(() => {
    ...
    setLoading(true)
    const ref = doc(db, 'workspaces', workspaceId, 'restaurantReservationSettings', 'default')
    const unsub = onSnapshot(ref, (snap) => {
      setSettings({ ...RESERVATION_SETTINGS_DEFAULTS, ...(snap.exists() ? snap.data() : {}) })
      setLoading(false)
    }, (err) => { setSettings(RESERVATION_SETTINGS_DEFAULTS); setLoading(false); setError(clientSafeMessage(err, 'Unable to load settings.')) })
    return () => unsub()
  }, [enabled, workspaceId])
  ...
  const saveSettings = useCallback(async (patch = {}) => {
    ...
    try {
      await setDoc(doc(db, 'workspaces', workspaceId, 'restaurantReservationSettings', 'default'),
        { ...RESERVATION_SETTINGS_DEFAULTS, ...patch, workspaceId, businessType, updatedBy: userId || '', updatedAt: serverTimestamp() },
        { merge: true })
      return { ok: true }
